    ...
    getAllChats(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const providerId = (_b = req.query) === null || _b === void 0 ? void 0 : _b.providerId;
                if (!userId || !providerId) {
                    res.status(statusCodes_1.default.BAD_REQUEST).json({ success: false, message: "Please provide the id" });
                }
                const response = yield this.chatInteractor.getAllChatsUseCase(userId, providerId);
                if (!response.success) {
                    res.status(statusCodes_1.default.BAD_REQUEST).json({ success: response.success, message: response.message });
                }
                res.status(statusCodes_1.default.OK).json({ success: response.success, chatData: response.chatData });
            }
            catch (error) {
